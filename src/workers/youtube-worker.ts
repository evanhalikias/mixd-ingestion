...
export class YouTubeWorker extends BaseIngestionWorker {
  ...
  private async fetchChannelVideos(
    ...
    return validMixes.map((video: any) => this.mapVideoToRawMix(video));
  }
  ...
  async fetchVideoData(videoId: string): Promise<RawMix[]> {
    try {
      ...
      return videos
        .filter((video: any) => this.isValidMix(video))
        .map((video: any) => this.mapVideoToRawMix(video));
  ...
  private mapVideoToRawMix(video: any): RawMix {
    ...
    // Use the first performing artist, or fallback to channel
    const primaryArtist = artistExtraction.performingArtists.length > 0 
      ? artistExtraction.performingArtists[0]
      : video.snippet.channelTitle;
